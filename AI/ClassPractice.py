...
class Chaotic_AI():
    ...
    def get_turn_from_web(self):
        spot = int(get_url("http://127.0.0.1:5000/AI_turn"))
        if self.is_space_free(self.game, spot):
            self.game.board_status[int(spot) - 1] = self.ai_player.player_mark
        else:
            self.mark_spot_comp()